...
namespace
{
    ...
    {
        ASSERT("VULKAN", sem == VK_NULL_HANDLE, "Semaphore::create called on already-created semaphore");
    ...
    {
        ASSERT("VULKAN", sem != VK_NULL_HANDLE, "Semaphore::wait on invalid semaphore");
        ASSERT("VULKAN", semKind == Kind::Timeline, "Semaphore::wait requires a Timeline semaphore");
        ...
        ASSERT("VULKAN", false, "vkWaitSemaphores failed");
        return false;
    }

    void Semaphore::signal(const uint64_t value) const
    {
        ASSERT("VULKAN", sem != VK_NULL_HANDLE, "Semaphore::signal on invalid semaphore");
        ASSERT("VULKAN", semKind == Kind::Timeline, "Semaphore::signal requires a Timeline semaphore");
    ...
    {
        ASSERT("VULKAN", sem != VK_NULL_HANDLE, "Semaphore::value on invalid semaphore");
        ASSERT("VULKAN", semKind == Kind::Timeline, "Semaphore::value requires a Timeline semaphore");
    ...
    bool Semaphore::waitBatch(const VkSemaphore* handles, const uint64_t* values, const uint32_t count, const bool allSignaled, const uint64_t timeoutNs)
    {
        ...
        ASSERT("VULKAN", false, "vkWaitSemaphores (waitBatch) failed");
        return false;
    }
    ...
    {
        ASSERT("VULKAN", sems.size() == values.size(), "Semaphore::waitBatch size mismatch between semaphores and values");
        ...
        if (count <= batchSize)
        {
            ...
            {
                ASSERT("VULKAN", sems[i].semKind == Kind::Timeline, "Semaphore::waitBatch requires Timeline semaphores");
        ...
        {
            ASSERT("VULKAN", s.semKind == Kind::Timeline, "Semaphore::waitBatch requires Timeline semaphores");
